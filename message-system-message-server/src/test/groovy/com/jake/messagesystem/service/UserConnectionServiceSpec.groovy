...
class UserConnectionServiceSpec extends Specification {
    ...
    def setup() {
        ...
        userConnectionService = new UserConnectionService(cacheService, userService, userConnectionRepository, userConnectionLimitService, new JsonUtil())
    }
    ...
    def "사용자 연결 신청에 대한 테스트."() {
        ...
        'Already connected'   | new UserId(1) | 'userA'        | new UserId(2) | 'userB'        | new InviteCode('user2code') | new InviteCode('user2code')   | UserConnectionStatus.ACCEPTED     | Pair.of(Optional.of(new UserId(1)), 'Already connected with ' + targetUsername)
        'Already invited'     | new UserId(1) | 'userA'        | new UserId(2) | 'userB'        | new InviteCode('user2code') | new InviteCode('user2code')   | UserConnectionStatus.PENDING      | Pair.of(Optional.of(new UserId(1)), 'Already Invited to ' + targetUsername)
        'Already rejected'    | new UserId(1) | 'userA'        | new UserId(2) | 'userB'        | new InviteCode('user2code') | new InviteCode('user2code')   | UserConnectionStatus.REJECTED     | Pair.of(Optional.of(new UserId(1)), 'Already Invited to ' + targetUsername)
    ...
    def "사용자 연결 신청 수락에 대한 테스트."() {
        ...
        'Limit reached by other user' | new UserId(8) | 'userI'        | new UserId(7) | 'userH'        | Optional.of(new UserId(7)) | UserConnectionStatus.PENDING      | Pair.of(Optional.empty(), 'Connection limit reached by other user.')
    }
