...
public class RecordDispatcher {
    ...
    public <T extends RecordInterface> void dispatchRecord(T record) {
        final BaseRecordHandler<T> handler = (BaseRecordHandler<T>) handlerMap.get(record.getClass());
    ...
    private void prepareRecordHandlerMapping() {
        final Map<String, BaseRecordHandler> beanHandlers = listableBeanFactory.getBeansOfType(BaseRecordHandler.class);

        for (BaseRecordHandler handler : beanHandlers.values()) {
            final Class<? extends RecordInterface> recordClass = extractRecordClass(handler);
