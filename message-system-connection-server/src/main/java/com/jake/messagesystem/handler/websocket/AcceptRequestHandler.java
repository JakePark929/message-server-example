...
public class AcceptRequestHandler implements BaseRequestHandler<AcceptRequest> {
    ...
    public AcceptRequestHandler(KafkaProducer kafkaProducer, ClientNotificationService clientNotificationService) {
    ...
    public void handleRequest(WebSocketSession senderSession, AcceptRequest request) {
        ...
        kafkaProducer.sendRequest(new AcceptRequestRecord(acceptorUserId, request.getUsername()), () ->
                clientNotificationService.sendErrorMessage(senderSession, new ErrorResponse(MessageType.ACCEPT_REQUEST, "Accept failed."))
        );
    }
}
