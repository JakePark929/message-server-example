...
public class RequestDispatcher {
    ...
    public <T extends BaseRequest> void dispatchRequest(WebSocketSession webSocketSession, T request) {
        final BaseRequestHandler<T> handler = (BaseRequestHandler<T>) handlerMap.get(request.getClass());
    ...
    private void prepareRequestHandlerMapping() {
        final Map<String, BaseRequestHandler> beanHandlers = listableBeanFactory.getBeansOfType(BaseRequestHandler.class);

        for (BaseRequestHandler handler : beanHandlers.values()) {
            final Class<? extends BaseRequest> requestClass = extractRequestClass(handler);
